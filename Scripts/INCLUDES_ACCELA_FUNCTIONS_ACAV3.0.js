...
/*------------------------------------------------------------------------------------------------------/
| Program : INCLUDES_ACCELA_FUNCTIONS_ACA.js
| Event   : N/A
|
...
|
| Notes   : Needed to change these functions to use the cap model for ACA.  Once changed a comment will be
|			added above the function.  Who changed it and the date.
/------------------------------------------------------------------------------------------------------*/
...
// KG 6/17/2013 Fixed to work in ACA with cap model
function addParcelAndOwnerFromRefAddress(refAddress)  // optional capID
	...
	// debugObject(primaryParcelResult);
...
//05/13/2013 - ITJSM validated this to work with ACA.  There were no changes.
function addParent(parentAppNum) { //adds the current application to the parent
...
//ITJSM - 5/10/2013 - corrected to work with cap model
	...
	while (i.hasNext()) {
		...
		if (fields != null) {
			...
			while (iteFields.hasNext()) {
				...
				if (itemName != "") {
					if (useAppSpecificGroupName) {
						if (field.getCheckboxType().equals(itemGroup) && field.getCheckboxDesc().equals(itemName)) {
							return field.getChecklistComment();
						}
					} else {
						if (field.getCheckboxDesc().equals(itemName)) {
							return field.getChecklistComment();
						}
					}
				} else {
...
function loadAppSpecific4ACA(thisArr) {//Returns an associative array of App Specific Info
	// Optional second parameter, capId to load from capModel in this event
	var itemCap = capId;
	if (arguments.length >= 2) {
		itemCap = arguments[1]; // use cap ID specified in args
...
//ITJSM - 4/2/2014 - fixed this to work with ACA var tval tsmfldi.next(); removed .get
//ITJSM - 5/9/2013 - fixed this to work with ACA
//ITJSM - 4/2/2014 - fixed this to work with ACA var tval tsmfldi.next(); removed .get
function loadASITables4ACA() {
	...
	logDebug("ASI Table Array : No table associated.");
	}
	return numrows;
}
